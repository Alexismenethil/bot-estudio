...
- Active Trainer and Exam sessions do not have automatic cross-device discovery. If a student starts a Trainer or Exam session and later opens the app from another device/browser, the app will not automatically discover that active session; continuation is supported only from the same device/browser that started it, using the session id pointer stored in localStorage. General study data covered by FR-027 (courses, progress, flashcards, documents, question banks, and scores) remains centrally persisted and consistent across devices; this limitation applies only to discovering active Trainer/Exam sessions.
